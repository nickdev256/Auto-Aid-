...
async function getOrCreateProviderWallet(providerIdInput) {
  ...
  async (req, res) => {
    try {
      ...
      if (item.status === "pending" || item.status === "approved") {
        const wallet = await getOrCreateProviderWallet(item.providerId);

        wallet.pendingBalance = Math.max(
          0,
          Number(wallet.pendingBalance || 0) - Number(item.amount || 0)
        );
        wallet.availableBalance =
          Number(wallet.availableBalance || 0) + Number(item.amount || 0);
    ...
    try {
      ...
      if (item.status === "pending" || item.status === "approved") {
        ...
        wallet.totalWithdrawn =
          Number(wallet.totalWithdrawn || 0) + Number(item.amount || 0);
...
router.get("/wallet", async (req, res) => {
  try {
    const wallet = await getOrCreateProviderWallet(req.user._id);

    return res.json({
      totalEarned: Number(wallet.totalEarned || 0),
      pendingBalance: Number(wallet.pendingBalance || 0),
      totalPaidOut: Number(wallet.totalWithdrawn || 0),
...
router.post("/payout-requests", async (req, res) => {
  try {
    ...
    wallet.availableBalance = Math.max(
      0,
      Number(wallet.availableBalance || 0) - amount
    );
