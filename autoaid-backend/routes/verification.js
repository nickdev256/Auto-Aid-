...
function isValidDocumentType(value = "") {
  const allowedDocumentTypes = ["national_id", "passport", "drivers_license"];
...
function buildUser(req, userDoc) {
  ...
  return {
    ...
    verificationDocumentUrl: toPublicUrl(req, user?.verificationDocumentUrl || ""),
    profileImage: toPublicUrl(req, user?.profileImage || ""),

    workLicenseDocumentPath: user?.workLicenseDocumentUrl || "",
    businessRegistrationDocumentPath:
      user?.businessRegistrationDocumentUrl || "",
    nationalIdFrontPath: user?.nationalIdFrontUrl || "",
    nationalIdBackPath: user?.nationalIdBackUrl || "",
    ...
    return res.json({
      ...
      provider: {
        ...
        providerVerification: {
          ...
          documentType: user.verificationDocumentType || "",
          rejectionReason: user.verificationRejectionReason || "",
          licenseDocumentUrl: user.workLicenseDocumentUrl || "",
          businessDocumentUrl: user.businessRegistrationDocumentUrl || "",
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (rawDocumentType && !isValidDocumentType(normalizedDocumentType)) {
        return res.status(400).json({
          message: `Unsupported document type: ${rawDocumentType}`,
        });
      }
      ...
      if (normalizedDocumentType) {
        update.verificationDocumentType = normalizedDocumentType;
      }
      ...
      if (verificationDocumentFile) {
        update.verificationDocumentUrl = normalizePath(
          verificationDocumentFile.path
        );
      }
...
router.get("/admin/all", async (req, res) => {
  try {
    ...
    return res.json({
      message: "Provider verifications fetched successfully",
      providers: providers.map((p) => buildUser(req, p)),
    });
  } catch (e) {
    console.error("all provider verifications error:", e);
    return res.status(500).json({
      message: e.message || "Failed to fetch provider verifications",
    });
  }
});
...
router.get("/admin/:id", async (req, res) => {
  try {
    ...
    return res.json({
      message: "Provider verification fetched successfully",
      provider: buildUser(req, provider),
    });
  } catch (e) {
    console.error("single provider verification error:", e);
    return res.status(500).json({
      message: e.message || "Failed to fetch provider verification",
    });
  }
